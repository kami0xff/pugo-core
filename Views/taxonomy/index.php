 ...
 * Taxonomy View
 * 
 * Variables from TaxonomyController::index():
 * - $pageTitle, $view (tags|categories|keywords)
 * - $tags, $taxonomy, $keywords
 * - $message, $messageType
 * - $config, $currentLang, $contentDir
 */
?>
?>
...
<div class="content-wrapper">
    ...
    <div class="taxonomy-nav">
        <a href="?view=tags&lang=<?= $currentLang ?>" class="<?= $view === 'tags' ? 'active' : '' ?>">
            🏷️ Tags
        </a>
        <a href="?view=sections&lang=<?= $currentLang ?>" class="<?= $view === 'sections' ? 'active' : '' ?>">
            📁 Sections & Categories
        </a>
        <a href="?view=keywords&lang=<?= $currentLang ?>" class="<?= $view === 'keywords' ? 'active' : '' ?>">
            🔑 Keywords
        </a>
    </div>
    ...
    <?php if ($view === 'tags'): ?>
        ...
        <div class="tags-table-container">
            <table class="tags-table" id="tagsTable">
                ...
                <tbody>
                    <?php foreach ($tags as $tag_name => $tagData): ?>
                    <tr data-tag="<?= htmlspecialchars(strtolower($tag_name)) ?>">
                        ...
                        <td>
                            <span class="tag-count-badge"><?= $tagData['count'] ?></span>
                        </td>
                        <td>
                            <div class="tag-articles-preview">
                                <?php 
                                $shown = 0;
                                foreach ($tagData['articles'] as $article): 
                                    if ($shown >= 2) break;
                                    $relative = str_replace($contentDir . '/', '', $article['path']);
                                ?>
                                <div class="tag-article-preview">
                                    <a href="edit.php?file=<?= urlencode($relative) ?>&lang=<?= $currentLang ?>">
                                ...
                                if ($tagData['count'] > 2):
                                ?>
                                <span class="tag-more-link">+<?= $tagData['count'] - 2 ?> more</span>
                                <?php endif; ?>
                            </div>
                        </td>
                        <td>
                            <div class="tag-actions">
                                ...
                                <button class="tag-action-btn danger" onclick="openDeleteModal('<?= htmlspecialchars(addslashes($tag_name)) ?>', <?= $tagData['count'] ?>)">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        ...
                        <div class="category-articles" style="display: none;">
                            <?php foreach ($category['articles'] as $article): ?>
                                <div class="article-item">
                                    <a href="edit.php?file=<?= urlencode($article['relative_path']) ?>&lang=<?= $currentLang ?>" 
                                       class="article-title">
                                    ...
                                    <div class="article-tags">
                                        <?php foreach ($article['tags'] as $tag): ?>
                                            <a href="?view=tags&lang=<?= $currentLang ?>#tag-<?= urlencode($tag) ?>" 
    ...
    <?php elseif ($view === 'keywords'): ?>
        <!-- Keywords View -->
        <div class="tags-grid">
            <?php foreach ($keywords as $keyword_name => $keywordData): ?>
                <div class="tag-card">
                    <div class="tag-card-header">
                        <span class="tag-card-name">🔑 <?= htmlspecialchars($keyword_name) ?></span>
                        <span class="tag-card-count"><?= $keywordData['count'] ?></span>
                    </div>
                    <div class="tag-card-articles">
                        <?php foreach ($keywordData['articles'] as $article): ?>
                            <?php 
                                $relative = str_replace($contentDir . '/', '', $article['path']);
                            ?>
                            <a href="edit.php?file=<?= urlencode($relative) ?>&lang=<?= $currentLang ?>" 
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <select name="target_tag" id="mergeTargetTag" required>
                        ...
                        <?php foreach ($tags as $tag_name => $tagData): ?>
                            <option value="<?= htmlspecialchars($tag_name) ?>"><?= htmlspecialchars($tag_name) ?> (<?= $tagData['count'] ?> articles)</option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
