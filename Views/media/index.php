 ...
 * Media Library View
 * 
 * Variables from MediaController::index():
 * - $pageTitle: string
 * - $currentPath: string
 * - $media: array (files + directories)
 * - $uploadMessage: array|null
 * - $config: array
 */
?>
...
<div style="background: <?= $uploadMessage['type'] === 'success' ? 'rgba(16, 185, 129, 0.1)' : 'rgba(225, 29, 72, 0.1)' ?>; 
            border: 1px solid <?= $uploadMessage['type'] === 'success' ? '#10b981' : '#e11d48' ?>; 
            color: <?= $uploadMessage['type'] === 'success' ? '#10b981' : '#e11d48' ?>; 
            padding: 12px 16px; border-radius: 8px; margin-bottom: 24px;">
    <?= htmlspecialchars($uploadMessage['text']) ?>
</div>
<?php endif; ?>
...
<div class="card" style="margin-bottom: 24px; padding: 16px;">
    <div class="breadcrumb" style="margin: 0;">
        ...
        <a href="media.php?path=<?= urlencode($cumulativePath) ?>"><?= htmlspecialchars($part) ?></a>
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="directory" class="form-input" 
                       value="<?= htmlspecialchars($currentPath ?: 'articles') ?>"
...
<div id="newFolderModal" class="modal-overlay">
    <div class="modal">
        ...
        <form method="POST">
            <?= csrf_field() ?>
            <input type="hidden" name="parent" value="<?= htmlspecialchars($currentPath) ?>">
