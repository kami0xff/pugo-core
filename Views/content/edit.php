 ...
 * Content Edit View
 * 
 * Variables from ContentController::edit():
 * - $pageTitle, $file, $filePath, $frontmatter, $body
 * - $section, $category, $isIndex, $contentType, $contentTypeInfo
 * - $sectionColor, $sectionName, $translations
 * - $templateInfo, $allContent, $config, $currentLang
...
<div class="breadcrumb">
    ...
    <a href="articles.php?section=<?= $section ?>&lang=<?= $currentLang ?>"><?= ucfirst($section) ?></a>
    <?php if ($category): ?>
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        <polyline points="9 18 15 12 9 6"/>
    </svg>
...
<div class="article-info-bar">
    <div class="article-info-item">
        ...
        <span class="article-info-badge" style="background: <?= $sectionColor ?>20; color: <?= $sectionColor ?>;">
            <?= $sectionName ?>
        </span>
    </div>
    ...
    <div class="article-info-item">
        ...
        <span class="article-info-value">
            ...
            <?= $config['languages'][$currentLang]['name'] ?>
        </span>
    </div>
    ...
    <div class="article-info-item">
        ...
        <?php if ($templateInfo['exists']): ?>
        <a href="template-edit.php?file=<?= urlencode($templateInfo['path']) ?>" 
           ...
           title="Click to edit this template">
            <?= pugo_icon('code', 12) ?>
            <?= htmlspecialchars($templateInfo['path']) ?>
        </a>
        <?php else: ?>
        <span class="article-info-value" style="font-family: monospace; font-size: 12px; color: var(--text-muted);" title="Using Hugo default or theme template">
            <?= htmlspecialchars($templateInfo['path']) ?> (theme/default)
        </span>
        <?php endif; ?>
    </div>
...
<div class="page-header">
    <div>
        ...
        <p class="page-subtitle">
            Last modified: <?= time_ago(filemtime($filePath)) ?>
    ...
    <div style="display: flex; gap: 12px;">
        ...
        <a href="<?= $config['site_url'] ?>/<?= $currentLang !== 'en' ? $currentLang . '/' : '' ?><?= str_replace('.md', '/', $file) ?>" 
...
<div class="lang-tabs" style="margin-bottom: 24px;">
    <?php foreach ($config['languages'] as $lang => $langConfig): 
        $langExists = $lang === $currentLang || (isset($translations[$lang]) && $translations[$lang]['exists']);
        $langPath = isset($translations[$lang]['path']) ? str_replace(HUGO_ROOT . '/', '', $translations[$lang]['path']) : null;
    ?>
    ...
    <?php elseif ($langExists && $langPath): ?>
    <a href="edit.php?file=<?= urlencode(basename($langPath)) ?>&lang=<?= $lang ?>" class="lang-tab">
        <span class="flag"><?= $langConfig['flag'] ?></span>
        <?= $langConfig['name'] ?>
    </a>
    <?php else: ?>
    <a href="new.php?translate_from=<?= urlencode($file) ?>&source_lang=<?= $currentLang ?>&target_lang=<?= $lang ?>" 
...
<form method="POST" id="editForm">
    ...
    <?= render_content_type_fields($contentType, $frontmatter) ?>
    ...
    <div class="card" style="margin-top: 24px;">
        ...
        <div class="form-group" style="margin-top: 16px; margin-bottom: 0;">
            ...
            <select id="relatedSelect" class="form-input" onchange="addRelatedArticle(this)">
                ...
                <?php 
                $allContent = get_all_articles_for_selection($currentLang);
                $current_sections = [];
                foreach ($allContent as $art): 
