<?php
/**
 * Pugo Core - Centralized Icon System
 * 
 * Single source of truth for all SVG icons used in Pugo.
 * Based on Feather Icons (https://feathericons.com)
 * 
 * Usage:
 *   Icons::render('home')           // Returns SVG string
 *   Icons::render('home', 24)       // With custom size
 *   Icons::render('home', 24, 'my-class') // With custom class
 *   Icons::all()                    // Get all icon names
 *   Icons::exists('home')           // Check if icon exists
 */

namespace Pugo\Assets;
...
class Icons
{
     ...
     * SVG attributes
     */
    private const SVG_ATTRS = 'viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"';
    ...
    private const ICONS = [
        ...
        'tags' => '<path d="M12 2H2v10l9.29 9.29c.94.94 2.48.94 3.42 0l6.58-6.58c.94-.94.94-2.48 0-3.42L12 2Z"/><path d="M7 7h.01"/>',
    ...
    public static function render(
        ...
        $path = self::ICONS[$name] ?? self::ICONS['help-circle'];
        
        $attrString = self::SVG_ATTRS;
        $attrString .= " width=\"{$size}\" height=\"{$size}\"";
        ...
        return "<svg {$attrString}>{$path}</svg>";
    }
    
    /**
     * Get just the SVG path for an icon
     */
    public static function path(string $name): string
    {
        return self::ICONS[$name] ?? self::ICONS['help-circle'];
    }
    ...
    {
        return [
            'Navigation & UI' => ['home', 'menu', 'x', 'check', 'check-circle', 'x-circle', 'alert-circle', 'alert-triangle', 'info', 'help-circle', 'settings', 'sliders'],
            'Content & Files' => ['file', 'file-text', 'file-plus', 'folder', 'folder-open', 'copy', 'clipboard'],
            'Actions' => ['plus', 'plus-circle', 'minus', 'minus-circle', 'edit', 'edit-2', 'edit-3', 'trash', 'trash-2', 'save', 'download', 'upload', 'refresh-cw', 'rotate-cw'],
            'Media' => ['image', 'video', 'camera', 'music', 'play', 'pause', 'play-circle'],
            'Data & Database' => ['database', 'server', 'hard-drive', 'archive'],
            'Layout & Grid' => ['grid', 'layout', 'sidebar', 'columns', 'list'],
            'Navigation' => ['arrow-left', 'arrow-right', 'arrow-up', 'arrow-down', 'chevron-left', 'chevron-right', 'chevron-up', 'chevron-down', 'external-link'],
            'User & People' => ['user', 'users', 'user-plus', 'user-minus', 'user-check'],
            'Search & View' => ['search', 'zoom-in', 'zoom-out', 'eye', 'eye-off', 'filter'],
            'Communication' => ['mail', 'message-square', 'message-circle', 'bell', 'phone'],
            'Security' => ['lock', 'unlock', 'shield', 'shield-off', 'key'],
            'Social & Misc' => ['heart', 'star', 'bookmark', 'flag', 'share', 'share-2'],
            'Development' => ['code', 'terminal', 'git-branch', 'git-commit', 'git-merge', 'git-pull-request'],
            'Misc' => ['globe', 'map', 'map-pin', 'calendar', 'clock', 'link', 'link-2', 'tag', 'tags', 'zap', 'compass', 'book', 'book-open', 'building', 'box', 'package', 'layers', 'log-in', 'log-out', 'more-horizontal', 'more-vertical'],
        ];
    }
    ...
    {
        return count(self::ICONS);
    }
}

/**
 * Global helper function for rendering icons
 * 
 * @param string $name Icon name
 ...
 * @return string SVG HTML
 */
function icon(string $name, int $size = 24, ?string $class = null): string
{
